...
def prune_adj(oriadj, non_zero_idx, percent):
    ...
    low_adj= np.tril(adj, -1)  ### get the lower triangular matrix. np.tril(m,k). k indicates the diagonal about which the LTM is created. Default 0.
...
#### returns the mask flattened with only those locations where an edge/weight is present.
...
def get_final_mask_epoch(model, adj_percent, wei_percent):
    ...
    ### sort
    adj_y, adj_i = torch.sort(adj_mask.abs()) ## returns the sorted values and the indexes.
...
##### adjacency mask has 1 where there is an edge otherwise it is 0.
...
def soft_mask_init(model, init_type, seed):
    ...
    if init_type == 'all_one':
        ...
        model.adj_mask1_train.requires_grad = False
        model.adj_mask1_train.mul_(model.adj_mask2_fixed) ### to make sure only the places where an edge is present is updated.
        model.adj_mask1_train.requires_grad = True
