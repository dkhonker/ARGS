...
def get_psu_labels(logits, pseudo_labels, idx_train, idx_test, k=30, append_idx=True):
    ...
    pred_labels = torch.argmax(logits, dim=1) ### predicted MLP label
    pred_labels_test = pred_labels[idx_test]  ### predicted MLP label for test nodes only.
    for label in range(pseudo_labels.max().item() + 1):
        # 
        idx_label = idx_test[pred_labels_test==label] ### for the given label; idx_label is the set of nodes of a given class
        logits_label = logits[idx_label][:, label] ## only the logit value for getting top k
    ...
    return idx_train, pseudo_labels ### pseudo_labels has the true label for the train and val nodes and predicted labels for the top k test nodes.
